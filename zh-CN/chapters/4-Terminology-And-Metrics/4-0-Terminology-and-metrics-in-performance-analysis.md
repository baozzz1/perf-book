...
本章是对性能分析中基本术语和指标的入门介绍。我们将首先定义基本概念，如已退休/已执行指令（retired/executed instructions）、IPC/CPI、$\mu$ops、核心/参考时钟（core/reference clocks）、缓存未命中（cache misses）和分支预测错误（branch mispredictions）。然后我们将了解如何测量系统的内存延迟（memory latency）和带宽（bandwidth），并介绍一些更高级的指标。最后，我们将对四个行业工作负载进行基准测试，并查看收集到的指标。
