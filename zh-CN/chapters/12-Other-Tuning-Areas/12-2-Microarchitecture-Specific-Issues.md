...
从硬件角度来看，处理次正规数比处理正规 FP 值更困难，因为它需要特殊处理，通常被视为异常情况。应用程序不会崩溃，但会受到性能惩罚。产生或使用次正规数的计算比处理正规数的类似计算慢，可能慢 10 倍或更多。例如，Intel 处理器目前通过微码辅助（microcode assist）处理次正规数的操作。当处理器识别到次正规 FP 值时，微码序列器（Microcode Sequencer，MSROM）将提供计算结果所需的微操作（μops）。
