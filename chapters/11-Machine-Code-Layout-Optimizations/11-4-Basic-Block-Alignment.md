...
Sometimes performance can significantly change depending on the offset at which instructions are laid out in memory. Consider a simple function presented in Listing LoopAlignment along with the corresponding machine code when compiled with `-O3 -march=core-avx2 -fno-unroll-loops` (loop unrolling is disabled to illustrate the idea).
...
void benchmark_func(int* a) {    │ 00000000004046a0 <_Z14benchmark_funcPi>:
  for (int i = 0; i < 32; ++i)   │ 4046a0: mov rax,0xffffffffffffff80
    a[i] += 1;                   │ 4046a7: vpcmpeqd ymm0,ymm0,ymm0
                                 ...
                                 │ 4046cf: ret 
```
The code itself is reasonable, but its layout is not perfect (see Figure Loop_default). Instructions that correspond to the loop are highlighted in yellow. Thick boxes denote cache line borders. Cache lines are 64 bytes long. 
